...
struct TodosTestSuite {
    ...
    func create(
        ...
        let buffer = try JSONEncoder().encodeAsByteBuffer(
            request,
    ...
    func testCreate() async throws {
        let app = try await buildTestApplication(TestServerConfiguration())
        try await app.test(.router) { client in
            let todo = try await self.create(
                title: "My first todo",
                client: client
            )
